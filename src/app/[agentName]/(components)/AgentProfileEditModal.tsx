...
const RichTextEditor = ({ value, onChange, placeholder }: RichTextEditorProps) => {
  ...
  const runToolbarAction = (e: React.MouseEvent<HTMLButtonElement>, action: () => void) => {
    // Keep focus and selection inside editor while clicking toolbar buttons.
    e.preventDefault();
    action();
  };
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        ...
        <button
          type="button"
          className={toolbarButtonClass}
          onMouseDown={(e) => runToolbarAction(e, insertBullets)}
        >
          Bullets
        </button>
        <button
          type="button"
          className={toolbarButtonClass}
          onMouseDown={(e) => runToolbarAction(e, clearEditor)}
        >
          Clear
        </button>
      </div>
